<script lang="ts">
    ...
    let {
        userId,
        onUserdeleteOrRestPass
    }: {
        userId: string,
        onUserdeleteOrRestPass: () => void
    } =$props()
    ...
    <p>Gérez les paramètres importants liés à ce compte utilisateur.</p>
        <Accordion flush> 
            <AccordionItem>
                ...
                <div class="flex justify-end pb-4 space-x-4 md:px-4">
                    <form use:enhance={handleEnhance} method="POST" action="?/userResetPassword" class="mt-3">
        ...
        <Accordion flush>
            <AccordionItem>
                ...
                <div class="flex justify-end pb-4 space-x-4 md:px-4">
                    <form use:enhance={handleEnhance} method="POST" action="?/userDelete" class="mt-3">
                        <input type="hidden" name="userId" value={userId}>
